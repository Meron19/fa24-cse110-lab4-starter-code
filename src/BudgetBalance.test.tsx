...
describe("Verify that budget is correcly updated", () => {
    test("Validate that the equation Budget = Remaining Balance + Total Expenditure holds true after various operations", () => {
      ...
      fireEvent.click(deleteButton1);
       ...
       const newExpenseName4 = screen.getByText("candy");
       const newExpenseCost4 = screen.getByText("$2");
 
       expect(newExpenseName4).toBeInTheDocument();
       expect(newExpenseCost4).toBeInTheDocument();

       //check if amount is correctly calculated
      const newRemaining4 = screen.getByText("Remaining: $18")
      expect(newRemaining4).toBeInTheDocument();

      const newspent4 = screen.getByText("Spent so far: $2");
      expect(newspent4).toBeInTheDocument();

      //delete expenses
      const deleteButton2 = screen.getAllByText("x")[0];
      fireEvent.click(deleteButton2);
       //check if amount is correctly calculated
       const newRemaining5 = screen.getByText("Remaining: $20")
       expect(newRemaining5).toBeInTheDocument();
 
       const newspent5 = screen.getByText("Spent so far: $0");
       expect(newspent5).toBeInTheDocument();
    });
  });